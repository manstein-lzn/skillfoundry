...
当前 WP0-WP8 已完成：项目已经具备离线可验证的 Codex Skill 工厂闭环，并且已经有可选的 CodexWorker pilot adapter。下一阶段是 WP9 Minimal API/UI，也就是给内部用户提供提交需求、查看报告、查询 Registry 和下载 approved Skill 的最小入口。
...
1. 启动 WP9 Minimal API/UI。
2. 提供最小 job create/get/list、report view、registry query 和 approved package download。
3. API 只能暴露 workspace 内安全路径，不允许下载未通过 Verifier 的 package。
4. UI 或静态报告页只展示 evidence refs、final status、Verifier 结果和 Registry 状态，不伪造 Codex 内部可 replay 能力。
5. WP9 稳定后，再推进 WP10 QA Lab Expansion。
