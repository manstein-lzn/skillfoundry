...
当前 WP0-WP9 已完成：项目已经具备离线可验证的 Codex Skill 工厂闭环、可选 CodexWorker pilot adapter，以及最小内部 API/UI 入口。下一阶段是 WP10 QA Lab Expansion，也就是把质量门从结构检查扩展到更接近真实可用性的 fixture、judge 和质量报告。
...
1. 启动 WP10 QA Lab Expansion。
2. 增加 trigger/non-trigger、输入输出契约、脚本 smoke 和回归 fixture。
3. 可增加 governed LLM judge，但不能让 judge 单独放行 Skill。
4. 输出质量报告和失败分类，让 repair loop 能根据失败原因改进。
5. WP10 稳定后，再推进 WP11 Feedback + Versioning。
