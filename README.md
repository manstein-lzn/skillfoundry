...
当前 WP0-WP11 已完成：项目已经具备离线可验证的 Codex Skill 工厂闭环、可选 CodexWorker pilot adapter、最小内部 API/UI 入口、QA Lab 质量报告层，以及反馈驱动的版本治理能力。下一阶段是 WP12 Production Hardening，也就是把现有闭环强化到小规模内部试用所需的稳定性和可观测性。
...
1. 启动 WP12 Production Hardening。
2. 增加并发写防护、artifact retention、workspace 清理和健康检查。
3. 输出安全清单、观测指标和小规模试运行报告。
4. 做 Python/Rust 边界和数据库迁移评估，不急于重写。
5. WP12 通过后，SkillFoundry 才进入内部 beta 试用标准。
