...
当前已经验证了第一版 adaptive steering MVP 和确定性 baseline/upgraded benchmark；
现在还提供一个显式 opt-in 的 `adaptive_codex` FrontDesk build mode，用同一套
adaptive loop 驱动 Codex-compatible command worker。默认路径仍保持 deterministic/offline。
...
Opt-in adaptive build path:

```text
FrontDesk
  -> adaptive_codex build mode
  -> CapabilityStateEstimate / RoutePlan / NextStepContract
  -> ForgeUnit Codex command worker
  -> ObservationReport / StateCorrection
  -> verifier / acceptance / ProductGradeGate evidence
...
Adaptive Codex build is also opt-in:

```json
{"build_mode": "adaptive_codex"}
```

Aliases `adaptive_forgeunit` and `forgeunit_adaptive` resolve to the same mode.
Without an explicit command, local tests use a deterministic fake worker; live
Codex still requires an explicit command boundary such as the wrapper documented
in the live eval runbook.
...
- [Adaptive Steering Substrate Extraction Plan](docs/ADAPTIVE_STEERING_SUBSTRATE_EXTRACTION_PLAN.md)
- [Git Worktree Workflow](docs/WORKTREE_WORKFLOW.md)
