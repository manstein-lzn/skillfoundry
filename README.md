...
1. 继续把 `graph_v2.py` 收敛为唯一的 build / verify / repair / registry 产品主骨架；当前默认 Front Desk frozen job 已能通过 `/frontdesk/jobs/{job_id}/build` 进入 graph v2 verified build / registry happy path，failed verification route 也能执行 Goal Harness-backed repair evidence node。
2. 补齐 repair 输出重新进入 verifier / acceptance coverage / registry gate 的闭环，确保 worker self-report 仍然不能成为验收或注册依据。
3. 完善 API/UI 对 v2 refs、ContextForge status、registry outcome、repair/human-review route 的消费，保持不泄漏 raw prompt / raw payload。
