...
当前 WP0-WP7 已完成：项目已经具备离线可验证的 Codex Skill 工厂闭环。下一阶段是 WP8，也就是在不破坏离线闭环、Verifier gate 和 Registry gate 的前提下，试点接入真实 Codex Worker。
...
1. 启动 WP8 CodexWorker Pilot。
2. 保持 Codex Worker 为外部黑盒 builder，不让 ContextForge 冒充其内部上下文控制器。
3. 通过 WorkerAdapter 捕获 invocation、transcript、diff、timeout、usage availability 和 failure class。
4. 继续以独立 Verifier 和 Registry gate 作为最终验收边界。
5. 在 WP8 试点稳定后，再推进 WP9 Minimal API/UI。
