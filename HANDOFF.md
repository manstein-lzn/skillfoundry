...
可以把 `JobWorkspace` 物化为 ForgeUnit task pack，并通过 ForgeUnit v1.2
的 public `ForgeUnitNode("codex_exec", dry_run=True)` 生成 refs-only v2
state。当前 dedicated pilot graph 会在 dry-run 后停在 human review，并写入
`contextforge/forgeunit_boundary_verification.json`、
`human_review/request.json` 和 `contextforge/forgeunit_pilot_graph_state.json`。
设计说明见 `docs/FORGEUNIT_PRODUCT_ADAPTER_SLICE.md`。
...
- `src/skillfoundry/forgeunit_adapter.py` 已创建第一层 ForgeUnit 产品适配：
  `JobWorkspace -> task.yaml -> ForgeUnitNode("codex_exec", dry_run=True) -> boundary verification -> human review`。
- v2 核心桥接模块已经存在：
