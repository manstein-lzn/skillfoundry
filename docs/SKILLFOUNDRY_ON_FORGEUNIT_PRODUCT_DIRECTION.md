...
SkillFoundry JobWorkspace
  -> task.yaml
  -> ForgeUnitNode("codex_exec", command=<local fake command>)
  -> package/SKILL.md + ForgeUnit evidence
...
并证明 ForgeUnit command-bridge 的产物可以被 SkillFoundry 既有 verifier /
registry 继续接管。dry-run 不会被当作 verifier 通过，也不会进入 registry。
当前 command-bridge 测试仍然是离线假命令，不是 live Codex。
后续再把真实 Codex exec、repair 和更完整的运行诊断接上。
...
1. 继续完成 ForgeUnit 内核：
   ...
   richer command diagnostics
   production-grade promotion record

2. 在 ForgeUnit 中做一个 SkillFoundry-style pilot task pack。

3. 在 SkillFoundry 中继续收敛 taskpack factory 和 verifier adapter。
