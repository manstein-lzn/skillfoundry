...
独立审查：已由第三方 `gpt-5.5 xhigh` reviewer 静态审查，结论为 `approve_with_required_clarifications`。审查要求已经合入本文：明确文档权威、限定第一阶段为离线 deterministic vertical slice、避免继续 patch 旧 `context.py` / `worker.py` / `llm_builder.py`。
...
本文是 SkillFoundry v2 重构的工程执行计划。

### 1.1 文档权威

从本文合入开始，SkillFoundry 后续 **v2 技术重建** 以本文为当前执行源。
...
| `HANDOFF.md` | 当前仓库状态和接手提醒，需要引用本文作为 v2 执行源。 |
...
### Phase 0: Entry Cleanup And Dependency Alignment
...
### Phase 5: Verification Bridge And Registry Gate
...
### Phase 7: API/UI And Productization
...
## 19. Immediate Next PR

The next code PR should be Phase 1 only:
...
If Phase 1 cannot produce valid ContextForge contracts from frozen SkillFoundry artifacts, do not proceed to worker migration.

## 20. Independent Reviewer Notes

Reviewer: third-party `gpt-5.5 xhigh`.
...
Required clarifications accepted into this document:

- 明确本文是 v2 技术重建当前执行源，旧 roadmap 只作为历史输入。
...
- README/HANDOFF updates that point v2 execution to this document；
