...
| `goal_runtime.py` | implemented / partial | 离线 verified Goal Harness runtime 已存在，并被 graph v2 API happy path 调用；repair Goal Harness runtime 能记录 WorkerRun、ContextView、PromptCachePlan、checkpoint、repair instructions、repair runtime result 和 `RepairAttempt`，并能把 repaired output 重新送入 verifier、acceptance coverage、ContextForge bridge 和 registry gate。 | 继续收敛 legacy 路径和 API/UI evidence。 |
...
| `graph_v2.py` | implemented / partial | refs-only spine、verified build/registry nodes、API happy path runner 已存在；failed verification route 可以执行 Goal Harness-backed repair node，并在 repaired output 通过 verifier/coverage/bridge 后进入 registry gate。 | 将旧 `graph.py` 退役或隔离为 compatibility wrapper，并完善 human-review/API-UI evidence。 |
...
Status: implemented / partial. `src/skillfoundry/graph_v2.py` exists and the Front Desk API happy path can now call graph v2 verified build / registry. Failed verification can now route into a Goal Harness-backed repair node, then repaired output can pass back through SkillFoundry verifier, acceptance coverage, ContextForge verification bridge, and the registry gate. Graph v2 is still not yet the only product build/verify/repair/registry path.
...
- Repaired output can be re-verified through SkillFoundry verifier, acceptance coverage, ContextForge verification bridge, and registry gate; failed repair verification routes to human review and does not register.
