...
| `src/skillfoundry/goal_runtime.py` | implemented | 离线 Goal Harness build/verify/registry runtime 已存在。 | 继续作为 graph v2 产品路径的 verified build node。 |
...
| `src/skillfoundry/graph_v2.py` | implemented / partial | refs-only spine、verified build/registry nodes、API happy path runner 已存在；仍不是唯一产品主骨架。 | 继续收敛 repair Goal Harness node，并将旧 `graph.py` 退役或隔离为 compatibility wrapper。 |
...
状态：happy path 已接入 API，仍不是唯一产品主路径。
...
- repair 仍是 planned route，不是完整 Goal Harness repair node。
