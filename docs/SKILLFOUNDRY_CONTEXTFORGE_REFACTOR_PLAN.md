...
- 当前下一步不是重做 ContextForge 或重做 Codex，而是让 graph v2 成为唯一产品 build / verify / repair / registry 主路径，并补齐 repair/human-review/API-UI evidence 体验。
...
| `src/skillfoundry/goal_runtime.py` | implemented / partial | 离线 Goal Harness build/verify/registry runtime 已存在；repair worker boundary 也能记录 WorkerRun、ContextView、PromptCachePlan、checkpoint 和 `RepairAttempt` 证据，但 repair 后续复验/注册闭环仍未完成。 | 继续作为 graph v2 产品路径的 verified build node，并补齐 repair -> verify -> registry 循环。 |
...
| `src/skillfoundry/graph_v2.py` | implemented / partial | refs-only spine、verified build/registry nodes、API happy path runner 已存在；failed verification route 可以进入 Goal Harness-backed repair node 并记录 refs-only repair evidence，但 graph v2 仍不是唯一产品主骨架。 | 补齐 repair 后的复验/注册循环，并将旧 `graph.py` 退役或隔离为 compatibility wrapper。 |
...
状态：happy path 已接入 API，repair route 已能进入 Goal Harness-backed repair node；仍不是唯一产品主路径。
...
- repair worker self-report 仍不是 acceptance，repair 结果不会自行 register；下一步需要把 repair 后续复验、acceptance coverage 和 registry gate 接回产品主循环。
