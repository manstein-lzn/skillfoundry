...
from skillfoundry.goal_runtime import GOAL_RUNTIME_LEDGER_REF, GOAL_RUNTIME_STATE_REF
...
from skillfoundry.schema import ArtifactRecord, ExecutionReport, sha256_file, utc_now
...
SKILLFOUNDRY_STATIC_REPORT_REF = "verifier/static_report.json"
SKILLFOUNDRY_SANDBOX_LOG_REF = "verifier/sandbox.log"
FRONTDESK_CONVERSATION_REF = "frontdesk/conversation.jsonl"
CONTEXTFORGE_FRONTDESK_BOUNDARY_EVIDENCE_REFS = [
    GOAL_CONTRACT_REF,
    BUILD_NODE_CONTRACT_REF,
    VERIFICATION_GATE_REF,
    CONTRACT_MANIFEST_REF,
    ...
    for ref in contract.expected_outputs:
        if ref == "package/SKILL.md":
            ...
            path.write_text(
                ...
                "\n## When To Use\n\n"
                "Use when validating adaptive graph fixture behavior.\n"
                "\n## When Not To Use\n\n"
                "Do not use as a production skill package.\n"
                "\n## Inputs\n\n"
                "- Frozen SkillFoundry workspace refs.\n"
                "\n## Outputs\n\n"
                "- A prompt-only fixture package.\n"
                "\n## Workflow\n\n"
                "1. Inspect the frozen refs.\n"
                "2. Produce the requested fixture artifact.\n"
                "\n## Safety\n\n"
...
def _execute_work_unit_node(config: AdaptiveGraphConfig, worker: AdaptiveWorkUnit) -> Any:
    def _node(state: SkillFoundryV2State) -> SkillFoundryV2State:
        ...
        manifest_refs = [work_unit_result_ref]
        refs = dict(state.get("refs", {}))
        refs["latest_work_unit_result"] = work_unit_result_ref
        contextforge = dict(state.get("contextforge", {}))
        if _should_materialize_adaptive_execution_report(workspace):
            execution_report_ref = adaptive_execution_report_ref(iteration)
            _write_adaptive_execution_report(workspace, iteration=iteration, result=result)
            manifest_refs.append(execution_report_ref)
            refs["latest_execution_report"] = execution_report_ref
            contextforge["adaptive_current_execution_report_ref"] = execution_report_ref
...
def _collect_observation_node(config: AdaptiveGraphConfig) -> Any:
    def _node(state: SkillFoundryV2State) -> SkillFoundryV2State:
        ...
        skillfoundry_verifier_ran = False
        if _should_run_skillfoundry_verifier(workspace, bundle_result):
            try:
                boundary_evidence_refs = _prepare_contextforge_frontdesk_boundary_evidence_for_verifier(workspace)
                verifier_result = Verifier().verify(workspace, attempt_id=f"{iteration:03d}")
            except Exception as exc:
                failures.append(f"skillfoundry_verifier_error: {exc}")
                contextforge["adaptive_skillfoundry_verification_passed"] = False
                contextforge["adaptive_skillfoundry_verification_error"] = type(exc).__name__
                verification_status = "failed"
            else:
                skillfoundry_verifier_ran = True
                _refresh_manifest_records(
                    workspace,
                    [
                        BUNDLE_VERIFICATION_RESULT_REF,
                        *boundary_evidence_refs,
                        ...
                        SKILLFOUNDRY_STATIC_REPORT_REF,
                        SKILLFOUNDRY_SANDBOX_LOG_REF,
                    ],
                    created_by="forgeunit_skillfoundry.adaptive_graph",
                )
                verifier_evidence = _dedupe_refs(
                    [
                        ...
                        *boundary_evidence_refs,
                        ...
                        SKILLFOUNDRY_STATIC_REPORT_REF,
                        SKILLFOUNDRY_SANDBOX_LOG_REF,
                        *verifier_result.evidence_refs,
                    ]
                )
                if boundary_evidence_refs:
                    contextforge["adaptive_contextforge_frontdesk_boundary_evidence_prepared"] = True
                    contextforge["adaptive_contextforge_frontdesk_boundary_refs"] = boundary_evidence_refs
                contextforge["adaptive_skillfoundry_verification_result_ref"] = SKILLFOUNDRY_VERIFICATION_RESULT_REF
                contextforge["adaptive_skillfoundry_verification_passed"] = verifier_result.passed
                if verifier_result.passed:
                    if verification_status != "failed":
                        verification_status = "passed"
                else:
                    verification_status = "failed"
                    failures.extend(f"skillfoundry_verifier:{failure}" for failure in verifier_result.failures)
        acceptance_coverage_ran = False
        if _should_run_acceptance_coverage(workspace):
            try:
                acceptance_plan = AcceptanceCriteriaPlanner().plan(workspace)
                acceptance_result = AcceptanceCoverageEvaluator().evaluate(workspace, plan=acceptance_plan)
            except Exception as exc:
                failures.append(f"acceptance_coverage_error: {exc}")
                contextforge["adaptive_acceptance_coverage_passed"] = False
                contextforge["adaptive_acceptance_coverage_error"] = type(exc).__name__
                verification_status = "failed"
            else:
                acceptance_coverage_ran = True
                _refresh_manifest_records(
                    workspace,
                    ACCEPTANCE_COVERAGE_REFS,
                    created_by="forgeunit_skillfoundry.adaptive_graph",
                )
                verifier_evidence = _dedupe_refs([*verifier_evidence, *ACCEPTANCE_COVERAGE_REFS])
                contextforge["adaptive_acceptance_coverage_plan_ref"] = ACCEPTANCE_COVERAGE_PLAN_REF
                contextforge["adaptive_acceptance_coverage_result_ref"] = ACCEPTANCE_COVERAGE_RESULT_REF
                contextforge["adaptive_acceptance_coverage_passed"] = acceptance_result.passed
                if acceptance_result.passed:
                    contextforge["adaptive_acceptance_coverage_status"] = "passed"
                else:
                    contextforge["adaptive_acceptance_coverage_status"] = "failed"
                    verification_status = "failed"
                    failures.extend(f"acceptance_coverage:{failure}" for failure in acceptance_result.failures)
        ...
        if skillfoundry_verifier_ran:
            refs["skillfoundry_verification_result"] = SKILLFOUNDRY_VERIFICATION_RESULT_REF
        next_state: SkillFoundryV2State = dict(state)
        next_state.update({"refs": refs, "contextforge": contextforge})
        validate_v2_graph_state(next_state)
        return next_state
...
def adaptive_execution_report_ref(iteration: int) -> str:
    if not isinstance(iteration, int) or isinstance(iteration, bool) or iteration <= 0:
        raise ForgeUnitSkillFoundryError("execution report iteration must be a positive integer")
    return f"attempts/{iteration:03d}/execution_report.json"
...
def _write_adaptive_execution_report(
    ...
    *,
    ...
    result.validate()
    attempt_id = f"{iteration:03d}"
    report_ref = adaptive_execution_report_ref(iteration)
    report_path = _optional_workspace_path(workspace, report_ref)
    if report_path.is_file():
        try:
            existing_report = ExecutionReport.read_json_file(report_path)
        except Exception as exc:
            raise ForgeUnitSkillFoundryError(f"existing adaptive execution report is invalid: {report_ref}: {exc}") from exc
        if existing_report.job_id != workspace.job_id or existing_report.attempt_id != attempt_id:
            raise ForgeUnitSkillFoundryError(f"existing adaptive execution report identity mismatch: {report_ref}")
        return
    workspace.resolve_path(f"attempts/{attempt_id}").mkdir(parents=True, exist_ok=True)
    failed = bool(result.failures) or result.verification_status == "failed"
    timestamp = utc_now()
    artifacts = _dedupe_refs(
        [
            *result.produced_artifacts,
            *result.changed_refs,
            adaptive_work_unit_result_ref(iteration),
        ]
    )
    report = ExecutionReport(
        report_id=f"{workspace.job_id}:adaptive-work-unit:{attempt_id}",
        invocation_id=f"{workspace.job_id}:adaptive-work-unit:{attempt_id}",
        job_id=workspace.job_id,
        ...
        duration_ms=0,
        exit_status="failure" if failed else "success",
        summary=(
            f"Adaptive work unit {attempt_id} completed with "
            f"{len(result.produced_artifacts)} produced artifact(s)."
        ),
        artifacts=artifacts,
        failures=list(result.failures),
    )
    report.write_json_file(workspace.resolve_path(report_ref))
...
def _build_next_step_contract(
    ...
    *,
    ...
    task_contract_refs = _visible_frontdesk_task_contract_refs(workspace)
    task_contract_metadata = _frontdesk_task_contract_metadata(workspace)
    skillfoundry_verification_refs = _visible_skillfoundry_verification_refs(workspace)
    skillfoundry_verification_metadata = _skillfoundry_verification_metadata(workspace)
    acceptance_coverage_refs = _visible_acceptance_coverage_refs(workspace)
    acceptance_coverage_metadata = _acceptance_coverage_metadata(workspace)
    if route == "repair":
        product_repair_contract = _build_product_repair_next_step_contract(
            ...
            task_contract_refs=task_contract_refs,
            task_contract_metadata=task_contract_metadata,
            skillfoundry_verification_refs=skillfoundry_verification_refs,
            skillfoundry_verification_metadata=skillfoundry_verification_metadata,
            acceptance_coverage_refs=acceptance_coverage_refs,
            acceptance_coverage_metadata=acceptance_coverage_metadata,
        )
        ...
        outputs = _repair_expected_outputs(workspace, attempt_dir)
    ...
    return NextStepContract(
        ...
        visible_refs=[
            ...
            ADAPTIVE_CAPABILITY_STATE_REF,
            *task_contract_refs,
            *skillfoundry_verification_refs,
            *acceptance_coverage_refs,
        ...
        metadata={
            ...
            **skillfoundry_verification_metadata,
            **acceptance_coverage_metadata,
        },
    )
...
def _build_product_repair_next_step_contract(
    ...
    *,
    ...
    task_contract_metadata: dict[str, Any],
    skillfoundry_verification_refs: list[str],
    skillfoundry_verification_metadata: dict[str, Any],
    acceptance_coverage_refs: list[str],
    acceptance_coverage_metadata: dict[str, Any],
    ...
    return NextStepContract(
        ...
        expected_outputs=[
            "package/SKILL.md",
            f"{attempt_dir}/repair_evidence.md",
        ],
        exit_criteria=[
...
def _visible_frontdesk_task_contract_refs(workspace: JobWorkspace) -> list[str]:
    path = _optional_workspace_path(workspace, FRONTDESK_TASK_CONTRACT_REF)
    return [FRONTDESK_TASK_CONTRACT_REF] if path.is_file() else []


def _frontdesk_task_contract_metadata(workspace: JobWorkspace) -> dict[str, Any]:
    if not _visible_frontdesk_task_contract_refs(workspace):
        return {}
    return {
        "frontdesk_task_contract_ref": FRONTDESK_TASK_CONTRACT_REF,
        "task_contract_is_frontdesk_standard": True,
    }


def _visible_skillfoundry_verification_refs(workspace: JobWorkspace) -> list[str]:
    refs = [
        SKILLFOUNDRY_VERIFICATION_RESULT_REF,
        SKILLFOUNDRY_STATIC_REPORT_REF,
        SKILLFOUNDRY_SANDBOX_LOG_REF,
    ]
    return [ref for ref in refs if _optional_workspace_path(workspace, ref).is_file()]


def _skillfoundry_verification_metadata(workspace: JobWorkspace) -> dict[str, Any]:
    if not _visible_skillfoundry_verification_refs(workspace):
        return {}
    return {
        "skillfoundry_verification_result_ref": SKILLFOUNDRY_VERIFICATION_RESULT_REF,
        "skillfoundry_verifier_is_acceptance_gate": True,
    }


def _visible_acceptance_coverage_refs(workspace: JobWorkspace) -> list[str]:
    return [ref for ref in ACCEPTANCE_COVERAGE_REFS if _optional_workspace_path(workspace, ref).is_file()]


def _acceptance_coverage_metadata(workspace: JobWorkspace) -> dict[str, Any]:
    refs = _visible_acceptance_coverage_refs(workspace)
    if not refs:
        return {}
    metadata: dict[str, Any] = {"acceptance_coverage_is_acceptance_gate": True}
    if ACCEPTANCE_COVERAGE_PLAN_REF in refs:
        metadata["acceptance_coverage_plan_ref"] = ACCEPTANCE_COVERAGE_PLAN_REF
    if ACCEPTANCE_COVERAGE_RESULT_REF in refs:
        metadata["acceptance_coverage_result_ref"] = ACCEPTANCE_COVERAGE_RESULT_REF
    return metadata


def _repair_expected_outputs(workspace: JobWorkspace, attempt_dir: str) -> list[str]:
    outputs = [f"{attempt_dir}/repair_evidence.md"]
    failures = _latest_skillfoundry_verifier_failures(workspace)
    acceptance_failures = _latest_acceptance_coverage_failures(workspace)
    failure_text = "\n".join([*failures, *acceptance_failures]).lower()
    if not failure_text:
        return outputs
    if any(
        ...
        outputs.insert(0, "package/SKILL.md")
    if any(
        marker in failure_text
        for marker in (
        ...
        outputs.insert(0, "package/src/lib.rs")
        outputs.insert(0, "package/Cargo.toml")
    if any(marker in failure_text for marker in ("bundle_manifest", "bundle_entrypoint", "bundle_prompt_only")):
        outputs.insert(0, BUNDLE_MANIFEST_REF)
    if acceptance_failures:
        outputs.insert(0, "package/SKILL.md")
    if "package/**/tests/fixtures" in failure_text or "fixture" in failure_text:
        outputs.append("package/tests/fixtures/synthetic/valid_evidence_manifest.json")
    if "package/**/tests/**/*.rs" in failure_text or "rust verifier test" in failure_text:
        outputs.append("package/tests/helper_contract.rs")
    if (
        "package/references" in failure_text
        or "reference documentation" in failure_text
        or "acceptance coverage" in failure_text
        or "evidence manifest" in failure_text
    ):
        reference_ref = (
            "package/references/codexarium_reference.md"
            if "codexarium" in failure_text
            else "package/references/reference.md"
        )
        outputs.extend(
            [
                reference_ref,
                ...
                "package/examples/write_plan.json",
                "package/examples/note_draft.md",
            ]
        )
    return _dedupe_refs(outputs)


def _latest_skillfoundry_verifier_failures(workspace: JobWorkspace) -> list[str]:
    path = _optional_workspace_path(workspace, SKILLFOUNDRY_VERIFICATION_RESULT_REF)
    if not path.is_file():
        return []
    try:
    ...
    if isinstance(failures, list):
        return [item for item in failures if isinstance(item, str) and item]
    return []


def _latest_acceptance_coverage_failures(workspace: JobWorkspace) -> list[str]:
    path = _optional_workspace_path(workspace, ACCEPTANCE_COVERAGE_RESULT_REF)
    if not path.is_file():
        return []
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
...
def _should_run_acceptance_coverage(workspace: JobWorkspace) -> bool:
    return (
        _optional_workspace_path(workspace, ACCEPTANCE_CRITERIA_REF).is_file()
        and _optional_workspace_path(workspace, SKILLFOUNDRY_VERIFICATION_RESULT_REF).is_file()
    )


def _should_run_skillfoundry_verifier(workspace: JobWorkspace, bundle_result: Any) -> bool:
    if not _should_materialize_adaptive_execution_report(workspace):
        return False
    if _product_acceptance_matrix_exists(workspace):
        return False
    if not _optional_workspace_path(workspace, "package/SKILL.md").is_file():
        return False
    return (
        getattr(bundle_result, "manifest_status", None) == "valid"
        and getattr(bundle_result, "passed", None) is True
    )


def _should_materialize_adaptive_execution_report(workspace: JobWorkspace) -> bool:
    return (
        _optional_workspace_path(workspace, FRONTDESK_TASK_CONTRACT_REF).is_file()
        or _optional_workspace_path(workspace, FRONTDESK_CONVERSATION_REF).is_file()
        or _optional_workspace_path(workspace, ACCEPTANCE_CRITERIA_REF).is_file()
        or _package_declares_full_verifier_runtime_contract(workspace)
    )


def _package_declares_full_verifier_runtime_contract(workspace: JobWorkspace) -> bool:
    path = _optional_workspace_path(workspace, "package/SKILL.md")
    if not path.is_file():
        return False
    try:
        ...
        return False
    return "cargo" in text or "rust" in text


def _prepare_contextforge_frontdesk_boundary_evidence_for_verifier(workspace: JobWorkspace) -> list[str]:
    if not _optional_workspace_path(workspace, FRONTDESK_CONVERSATION_REF).is_file():
        return []
    from skillfoundry.forgeunit_adapter import (
        _maybe_write_contextforge_frontdesk_boundary_evidence,
        _upsert_existing_manifest_records,
    )

    _maybe_write_contextforge_frontdesk_boundary_evidence(workspace, created_at=None)
    _upsert_existing_manifest_records(
        workspace,
        CONTEXTFORGE_FRONTDESK_BOUNDARY_EVIDENCE_REFS,
        created_by="forgeunit_skillfoundry.adaptive_graph",
    ...
    return [
        ref
        for ref in CONTEXTFORGE_FRONTDESK_BOUNDARY_EVIDENCE_REFS
