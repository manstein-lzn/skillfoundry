...
def sample_objects():
    ...
    return [
        ...
        FreezeManifest(
            ...
            task_contract_ref="frontdesk/task_contract.json",
        ...
        TaskContract(
            job_id="job-1",
            round_index=1,
            semantic_lock_ref="frontdesk/product_semantic_lock.json",
            semantic_lock_hash=HASH_A,
            semantic_summary="The user wants a weekly update writer.",
            requirement_clauses=["The user wants a weekly update writer."],
            skill_spec_ref="skill_spec.yaml",
            acceptance_criteria_ref="acceptance_criteria.yaml",
            verification_spec_ref="verification_spec.yaml",
            worker_input_ref="worker_input.md",
            ...
            execution_boundary={
                ...
                "blocked_paths": [".."],
                "required_artifacts": ["skill_spec.yaml", "frontdesk/task_contract.json"],
                "timeout_seconds": 300,
                "attempt_limit": 1,
                "locked_input_hashes_ref": "build_contract.yaml#locked_input_hashes",
            },
            product_identity_terms=["weekly update writer"],
            domain_terms=["markdown"],
            implementation_requirements=["Use only user-provided notes."],
            delivery_requirements=["Markdown weekly update."],
            must_not=["Read external systems."],
            semantic_lock_available=True,
            semantic_lock_truncated=False,
            omission_warnings=[],
            source_turn_ids=["turn-001"],
            source_char_count=42,
            sanitized_char_count=42,
            redaction_applied=False,
            source_trace=[{"turn_id": "turn-001", "source_ref": "frontdesk/conversation.jsonl"}],
        ),
        FrontDeskState(
            job_id="job-1",
